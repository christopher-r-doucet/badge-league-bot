...
async def create_league(interaction: Interaction, league_name: str):
    try:
        ...
        if not db.cursor.fetchone():
            raise ValueError("You must register first using /register before creating a league!")
        ...
        db.conn.commit()
        await interaction.response.send_message(f"Successfully created league '{league_name}'! You have been automatically added as a member.")
...
async def join_league(interaction: Interaction, league_name: str):
    try:
        ...
        db.cursor.execute('SELECT 1 FROM players WHERE discord_id = ?', (interaction.user.id,))
        if not db.cursor.fetchone():
            raise ValueError("You must register first using /register before joining a league!")
...
async def list_leagues(interaction: Interaction):
    try:
        ...
        if not leagues:
            await interaction.response.send_message("No leagues exist yet! Create one using /create_league")
            return
